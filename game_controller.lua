...
function module.apply_start_state()
    ...
    if tas.start.seed_type == "seeded" then
        state.seed = tas.start.seeded_seed
        ...
        set_adventure_seed(tas.start.adventure_seed[1], tas.start.adventure_seed[2])
        -- The seeded seed does not affect adventure runs.
    end
    if tas.start.shortcut then
        state.quest_flags = state.quest_flags | common.flag_to_value(QUEST_FLAG.SHORTCUT_USED)
    end
    state.world_next = tas.start.world
    state.level_next = tas.start.level
    state.theme_next = tas.start.theme
    if tas.start.tutorial_race then
        ...
        state.speedrun_character = common_enums.PLAYER_CHAR:value_by_id(tas.start.tutorial_race_referee).ent_type_id
        ...
        state.world_start = tas.start.world
        state.level_start = tas.start.level
        state.theme_start = tas.start.theme
        state.speedrun_activation_trigger = false
    end
    state.items.player_count = tas.start.player_count
    for player_index = 1, CONST.MAX_PLAYERS do
        local player_char = common_enums.PLAYER_CHAR:value_by_id(tas.start.players[player_index])
        state.items.player_select[player_index].activated = player_index <= tas.start.player_count
...
function module.apply_level_snapshot(level_index)
    ...
    if not module.current.tas.start.adventure_seed then
...
local function on_post_level_gen()
    ...
    if module.current.current_level_index == 1 and module.current.tas.start.seed_type == "seeded" then
        if options.debug_print_load and not module.current.tas.start.adventure_seed then
        ...
        module.current.tas.start.adventure_seed = { part_1, part_2 - part_1 }
    end
...
local function on_pre_update_level()
    ...
    if module.mode ~= common_enums.MODE.FREEPLAY and (state.loading == 0 or state.loading == 3) then
        ...
        for player_index = 1, module.current.tas.start.player_count do
            local input
