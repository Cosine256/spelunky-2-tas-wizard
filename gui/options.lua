...
function module:draw_panel(ctx, is_window)
    ...
    ctx:win_separator_text("Pausing")
    ...
    ctx:win_section("More Info", function()
        ...
        ctx:win_text("The supported pause type for TAS Wizard is "..pause_lib.SUPPORTED_PAUSE_TYPE..". This is shown in Overlunky as the following \"Toggled pause flags\": \"Freeze updates\", \"Freeze game loop\", and \"Freeze input\". Other pause types are not supported by TAS Wizard and may fail to pause in certain situations or cause other problems.")
        ctx:win_text("The other pause options in Overlunky are generally safe to use, but some of them can still interfere with TAS Wizard pausing. For example, if you enable the pause option to ignore freezing on level screens, then TAS Wizard will be unable to pause on that screen type.")
        ctx:win_indent(-common_gui.INDENT_SECTION)
    end)
    if pause.pause_type ~= pause_lib.SUPPORTED_PAUSE_TYPE then
        ctx:win_text("Warning: Pause type is set to an unsupported value ("..pause.pause_type..").")
    end
    options.pause_type_force_supported = ctx:win_check("Force supported pause type", options.pause_type_force_supported)
    ctx:win_text("Automatically switch to the supported pause type.")
    ...
    ctx:win_text("Instead of automatically pausing in a transition screen, temporarily suppress TAS inputs and wait for the user to exit the transition manually. This only affects automatic TAS Wizard pauses. This does not affect other sources of engine pauses, the game's pause menu, and entering a transition while already paused.")
