...
local function draw_tas_start_snapshot_settings(ctx, session, tas, is_options_tas)
    ...
    ctx:win_section("More info", function()
        ...
        ctx:win_text("TAS Wizard does not currently provide an editor for snapshot starts. The only built-in way to configure a snapshot start is via snapshot capture. However, you can still edit a snapshot start by modifying the saved TAS file in a text editor. The snapshot is stored in \"start_snapshot\". The structure of \"start_snapshot.state_memory\" matches the \"StateMemory\" type in Overlunky's documentation, though it only includes fields that are necessary for a screen snapshot. Be very careful when manually editing a TAS file. TAS Wizard has almost no file validation and a corrupted TAS file can cause many problems and errors.")
        ctx:win_indent(-module.INDENT_SECTION)
    end)
