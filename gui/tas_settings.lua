...
local function draw_tas_settings(ctx, tas)
    ...
    ctx:win_text("Player positions are used to show the player paths through each screen and to detect desyncs. If this setting is enabled, then player positions are saved as part of the TAS file, but this will greatly increase its file size. If disabled, then the TAS will always need to be played back once when loaded to store these positions in memory.")
    ...
    ctx:win_text("Screen snapshots allow the TAS to be played back from the start of the nearest screen instead of the start of the run. If this setting is enabled, then screen snapshots are saved as part of the TAS file, but this will greatly increase its file size. If disabled, then the TAS will always need to be played back once when loaded to store these snapshots in memory.")
    if ctx:win_button("Clear screen snapshots") then
        tas:clear_all_screen_snapshots()
    end
    ctx:win_text("Clears stored screen snapshot data. Use this if you believe the stored data is out of sync with the inputs. This data can be regenerated by playing back the TAS.")
